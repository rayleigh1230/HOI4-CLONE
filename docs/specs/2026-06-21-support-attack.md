...
### 5.3 UI 操作: 点击目标省弹菜单选择命令(手机+电脑通用)

**废弃**之前的 Ctrl+右键方案(手机端无右键/Ctrl 键, 不可用)。

采用触屏友好的**弹窗菜单**方案(手机电脑通用):
- 选中师后, 点击目标省 → 弹出小菜单, 选命令。
...
```
┌─────────────┐
│ ⚔️ 进军攻击  │  → engine_move_division (红箭头)
│ 🎯 支援攻击  │  → engine_support_attack (蓝箭头)
│ ✖️ 取消      │  → 关闭菜单, 不下令
└─────────────┘
```

实现改动(web/index.html):
1. 新增命令菜单 DOM 元素(`#orderMenu`), 绝对定位, 默认隐藏。
2. click 监听改造(Canvas 上):
   - 点省 + 有选中师 → 显示 `#orderMenu` 在点击坐标, 记录待下令的 `(selectedDivId, targetProv)`。
   - 点省 + 无选中师 → 选地块(原逻辑)。
   - 注: 师的"选中"通过侧边面板点师卡片完成(现有逻辑 line 348-352), 非 Canvas 点击。
     故 Canvas 点省不会误选中师, 只会触发下令/选地块。
3. 菜单项 click:
   - "进军攻击" → `engine_move_division(divId, targetProv)`, 关闭菜单。
   - "支援攻击" → `engine_support_attack(divId, targetProv)`, 关闭菜单。
   - "取消" → 关闭菜单。
   - 每项执行后 refresh()。
4. 点菜单外任意处 → 关闭菜单(点 Canvas/其他元素时检测菜单是否打开, 打开则关闭不下令)。
5. 地图提示更新: "点师选中 → 点省选命令(进军/支援)"。

**触屏适配**:
- click 事件在触摸设备上等价于 tap, 无需额外处理。
- 菜单按钮足够大(44×44px+, 触摸友好标准)。
- 菜单位置在触摸点附近, 不超出视口边缘。
