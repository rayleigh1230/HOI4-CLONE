...
fn serialize_state(world: &World) -> String {
    ...
    for d in world.divisions.values() {
        ...
        first = false;
        s.push_str(&format!(
            "{{\"id\":{},\"owner\":\"{}\",\"org\":{:.1},\"max_org\":{:.0},\"str\":{:.1},\"max_str\":{:.0},\"eq_ratio\":{:.2},\"mp_ratio\":{:.2},\"retreating\":{},\"annihilated\":{}}}",
            d.id, d.owner_tag, d.org, d.max_org, d.strength, d.max_strength,
            d.equipment_ratio_only(), d.manpower_ratio(), d.retreating, d.is_annihilated()
        ));
    }
    s.push_str("],\"battles\":");
